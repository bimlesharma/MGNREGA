/**
 * Optimized dashboard API using aggregation instead of pre-computed summaries
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check cache
    const cacheKey = `dashboard:v2:district:${districtCode}:${financialYear || 'all'}:${months}`;
    ...
    const latestData = await getLatestDistrictData(districtCode, financialYear || undefined);
    if (!latestData) {
      return NextResponse.json({
        error: 'No data available',
        message: `No MGNREGA data found for district "${district.districtName}"`,
        district: {
          code: districtCode,
